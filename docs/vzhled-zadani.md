...
| Karta směny | Tři stavy, každý s tvarem a slovem, ne jen barvou: vydaná (plný modrý rámeček), **Nevydáno** (přerušovaný jantarový + značka), **Změněno** po vydání (plný jantarový + značka). Původní stav je v `title` a v panelu. **Puntík u času** (8 px, jantarový, `.ds-smd-znacka`) říká jediné: tahle směna ještě není v rozpisu, který lidé dostali — je buď **Nevydáno** (nikdy nebyla rozeslaná), nebo **Změněno** po vydání. Vydaná směna ho nemá. Stav tak nese i tvar, ne jen barva a slovo pod časem; puntík je `aria-hidden`, protože slovo a `title` už stav řeknou. Nepleť si ho s tečkou ve sloupci jmen (`.ds-smd-tecka`): ta počítá nevydané změny člověka a vidí ji jen plánující. | `.ds-smd-smena`, `lib/rozpis-desktop.ts` (`stavSmeny`) |
