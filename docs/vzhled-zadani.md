...
| Zobrazit | Rychlá volba, co z rozpisu ukázat: **jedna pobočka** a pod ní **jeden úsek** (Plac, Kuchyně, Vedení…), plus „Všechny pobočky / Všechny úseky“. Tlačítko nese vybrané („Zobrazit │ Bernard · Plac“). Píše do týchž filtrů jako nabídka Filtry (`pobocky`, `useky`), takže se to ukáže jako čipy, počítá se do odznaku a jde to zrušit. Pobočka patří **směně**, ne člověku: kdo pracuje na dvou, má v každé jen její směny a hodiny. Lidé bez směny se při výběru pobočky neukazují (nemají pobočku, na které by stáli). Nabídne se, jen když je víc poboček nebo aspoň jeden úsek. | `desktop/nastroje.tsx`, `lib/rozpis-desktop.ts` (`smenaVyhovuje`, `sestavitMrizku`) |
...
| Import a export | Import z tabulky je tlačítko v záhlaví (panel s průvodcem), ne trvalá karta. Export měsíce do Excelu (.xlsx) a PDF: `/api/smeny/export`, právo `shifts.manage`. **Tisk je A4 na výšku:** týdny pod sebe (sedm dnů od pondělí; trhaná směna má pod časem drobně „pauza 15–17“), týden se netrhá a víc týdnů se na stránku skládá, dokud se vejdou. Excel má listy Rozpis (po týdnech, pevné měřítko + ruční zalomení stránek, záhlaví a zápatí se stranami), Souhrn (na výšku) a Matice měsíce (řádek na člověka, 30 sloupců na šířku — jen pro třídění, na papír se nehodí). | `app/api/smeny/export/route.ts`, `lib/rozpis-export*.ts` |
