...
| Karta směny | Karta je jeden řádek: čas a puntík (pod časem jen „pauza 15–17“ a zkratka pobočky). Rámeček nese stav vůči vydanému rozpisu: vydaná (plný, barva pobočky), **Nevydáno** (přerušovaný jantarový), **Změněno** po vydání (plný jantarový). **Puntík u času** (Šéfík 20. 9. 2026; `.ds-smd-znacka`, `puntikSmeny` v `lib/rozpis-desktop.ts`): **červený = nevydáno** (koncept, i vydaná a pak změněná — lidé mají starou verzi), **žlutý = vydáno a nepotvrzeno**, **zelený (s fajfkou) = vydáno a potvrzeno**. Slovo se na kartě nepíše; je v `title`, v `aria-label` (puntík je `aria-hidden`) a barvy vysvětluje **lišta nástrojů nad mřížkou** (jen tomu, kdo plánuje). „Potvrzeno“ je **skutečné potvrzení zaměstnancem**: v telefonu, v detailu své vydané směny, tlačítko **Potvrdit směnu** (u změny ho nese „Potvrdit změnu“ a potvrdí i směnu). Zapíše se do `smeny_potvrzeni` (migrace `20260920120000`) funkcí `potvrdit_smenu`, která ověří, že je to jeho směna, vydaná a od vydání beze změny; záznam nese **opis směny** (pobočka, den, časy, pauza) a potvrzuje se právě to znění, které člověk vidí (vedoucí ji mezitím změnil → potvrzení se odmítne, obrazovku je třeba obnovit). Potvrzení platí, jen dokud se směna s opisem shoduje — změna času, dne, pauzy, pobočky nebo člověka ho tiše zneplatní (směna je zase žlutá) a vrátí-li se změna zpátky, platí zas; vydání beze změn ho neruší. Vydaná neobsazená směna, směna zaměstnance **bez účtu** (nemá jak potvrdit) a žlutý u směn **v minulosti** puntík nemají. Vedoucí čte potvrzení jednou za okno přímo z tabulky (RLS: člověk své, vedoucí SOUČASNÉ pobočky směny, kde plánuje `shifts.manage`); nemá-li databáze tabulku, nebo se čtení nepovede, vydané směny puntík nemají a lišta to řekne — „nepotvrzeno“ se nevymýšlí. V panelu ke směně je řádek „Potvrzení směny“. (První návrh četl potvrzení z upozornění; kontrola ukázala, že to nejde — hromadné „přečíst vše“, upozornění vznikající už při uložení konceptu, přeřazení směny a import z Excelu by dávaly falešnou zelenou.) Tečka ve sloupci jmen (`.ds-smd-tecka`) je červená ze stejného důvodu: počítá nevydané změny člověka. | `.ds-smd-smena`, `lib/rozpis-desktop.ts` (`stavSmeny`, `puntikSmeny`), `app/[rozsah]/smeny/potvrzeni-okna.ts`, `app/[rozsah]/smeny/potvrzeni.ts` (`potvrditSmenu`), `app/[rozsah]/smeny/mobil/detail.tsx` |
...
| Pobočky | **Jeden řádek na člověka** (Šéfík 20. 9. 2026): kdo má v okně směny na dvou pobočkách, se v mřížce neobjeví dvakrát a má jeden součet — týž, jaký o něm říká export. Z toho plyne, co je nahoře: **vidím-li jednu pobočku** (jeden podnik, pobočkový rozsah, nebo výběr v Zobrazit), je nahoře její pruh a pod ním úseky; **vidím-li jich víc najednou**, pruhy nejsou (člověk by pod žádný nepatřil) a pobočku nese **karta směny** — jako výrazná **zkratka** („Černá Perla“ → `ČP`), ne dlouhý název, který by kartu roztáhl; celý název je v `title`. Zkratka sedí vedle času jako **výrazná značka** (druhý řádek karty vpravo — vedle času by se s puntíkem stavu do úzké buňky nevešla a puntík by vylezl z karty ven). Zkratka je **první písmeno každého slova** názvu („Černá Perla“ → `ČP`, „Bernard“ → `B`); potkají-li se dvě, přidá se každému slovu další písmeno (`ČernPerl` / `ČervPerg`), protože dvě pobočky se stejnou zkratkou by se na kartě nedaly rozeznat. Počítá je `zkratkyPobocek` ze **všech** poboček firmy, ne jen z těch v okně, takže se táž pobočka nezkracuje pokaždé jinak — a export je má z téhož zdroje, takže obrazovka a papír říkají totéž. Pruh pobočky se kreslí **vždy**, když je pobočka jediná (Šéfík 19. 9. 2026: „potřebuji zobrazit pobočku a pod ní jednotlivé úseky“) — dřív se při jedné schovával, takže po výběru pobočky v nabídce Zobrazit zmizel právě ten nadpis. Je **větší a těžší než hlavička úseku** (14,5 px/800 proti 12 px/700) — pobočka je nad úseky, tak to tak má i vypadat. Nese počet lidí a hodin jako hlavičky úseků a **dá se sbalit celý**, i s úseky pod ním. Hodiny jsou jen lidí, bez neobsazených směn — jinak by součet poboček nesouhlasil s „Celkem“ v patičce. | `desktop/mrizka.tsx`, `lib/rozpis-desktop.ts` (`PobockaMrizky`) |
...
| Import a export | Import z tabulky je tlačítko v záhlaví (panel s průvodcem), ne trvalá karta. Export měsíce do Excelu (.xlsx) a PDF: `/api/smeny/export`, právo `shifts.manage`. **Celý měsíc na jedné stránce A4 na výšku** (Excel i PDF): dny jsou v řádcích, lidé ve sloupcích (jméno a pozice otočené o 90°), sloupec je tak široký jako čas směny, který se píše krátce („8–16“, „16–23:30“). **Úseky ani pobočky se v tabulce nekreslí** — úseky určují jen pořadí lidí; kdo pracuje na víc pobočkách, má pobočku **pod časem směny** (celým názvem, když se vejde, jinak zkratkou vysvětlenou pod tabulkou; u jedné pobočky se nepíše vůbec). Trhaná směna má pod časem drobně „pauza 15–17“. **Jména v záhlaví jsou naležato, kdykoli se do sloupce vejdou** (Šéfík 20. 9. 2026) — zalomí se po slovech i na spojovníku („Jirásková-“ / „Novotná“), nejvýš na tři řádky. Otáčejí se o 90°, až když se nevejdou, a pak celé záhlaví najednou; půlka tak a půlka onak vypadá rozbitě. V sešitu se navíc sloupce lidí roztáhnou do zbytku stránky (strop 16 znaků), takže na A4 nezůstane úzký proužek vlevo a jméno se spíš vejde naležato. **Dělení lidí na stránky (listy) určuje jedna funkce, `vyberRozlozeni` v `lib/rozpis-rozlozeni.ts`, pro Excel i PDF** (Šéfík 20. 9. 2026) — dřív si sešit počítal dělení sám jen podle šířky a z týchž dat vycházelo jinak (25 lidí: PDF jedna stránka, sešit dva listy). Písmo se volí největší, při kterém se měsíc vejde (9 až 6 bodů); nevejde-li se ani při 6, lidé se rozdělí na víc stránek/listů (každý s celým měsícem), nebo — je-li to méně papíru — se měsíc zlomí na druhou stránku se záhlavím znovu. Sloupec sešitu je nejméně tak široký, aby se vešel nejdelší čas, pauza (drobným písmem) i součet hodin; měří se v bodech stejným metrem jako v PDF, ne podle počtu znaků, a součet hodin je 9 pt tučně, aby při 30 lidech nevycházelo písmo sešitu o půlku menší než v PDF. Zlomí-li se měsíc i v PDF, sešit se netiskne na jednu stránku, ale na šířku s opakovaným záhlavím (řádky 1–3, `Print_Titles`) a sloupce se nenatahují; kde přesně se řádky zlomí, si určí Excel. Excel má listy Rozpis (jinak přizpůsobeno na 1 × 1 stránku, záhlaví lidí otočené) a Souhrn (jméno, pozice, směny, hodiny). | `app/api/smeny/export/route.ts`, `lib/rozpis-rozlozeni.ts`, `lib/rozpis-export*.ts` |
| Nastavení směn | **Nastavení → Směny** (`settings.manage`) je nastavení celého modulu na jednom místě a rozcestník k ostatním nastavením kolem směn (Šablony, Úseky, Zařazení, Firma, Pobočky). Volba **„Nabízet výběr úseku / zařazení při přidávání směny“** (Šéfík 20. 9. 2026: zaměstnanci jsou zařazeni od začátku, změna se dá napsat do poznámky, výběr je zbytečné klikání): výchozí je zapnuto, tedy beze změny. Vypnutá schová pole ve formuláři nové směny a směna si vezme **zařazení zaměstnance** (`lib/smeny-formular.ts`); u upravované směny se stejným člověkem se zařazení nepřepíše a **u neobsazené směny se výběr nabízí vždy** (pozice tam říká, koho je třeba). Ukládá funkce `nastavit_smeny_formular` (migrace `20260920130000`, `settings.manage`, audit); stránka Rozpis čte sloupec tolerantně — bez migrace se pole nabízí jako dřív. | `app/[rozsah]/nastaveni/smeny/`, `lib/smeny-formular.ts`, `app/[rozsah]/smeny/formular-smeny.tsx` |
