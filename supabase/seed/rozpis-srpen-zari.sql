...
--     -B                → Bernard Bar Tábor
--     R = ranní, O = odpolední, X = celá směna
--     STANEK            → k směně se připíše poznámka "+ stánek"
--
-- POZOR — časy jsou ODHAD, ne zadání. V tabulce žádné nebyly:
--     ranní      07:00–15:30
--     odpolední  15:00–23:30
--     celá       10:00–22:00
-- Typ je zapsaný v poznámce u každé směny, takže oprava časů je
-- jeden UPDATE, ne přepisování 133 řádků. Například:
--     update public.shifts set starts_at='06:00', ends_at='14:00'
--      where note like 'rozpis ranní%';
--
-- NENAHRÁNO — čtyři buňky, jejichž význam neznáme:
--     26. a 27. 8.  Lucka  "D"
--     11. a 12. 9.  Láďa   "TAB"
--
...
begin
  ...
  select v_tenant,
         ...
         case z.typ when 'r' then time '07:00' when 'o' then time '15:00' else time '10:00' end,
         case z.typ when 'r' then time '15:30' when 'o' then time '23:30' else time '22:00' end,
         'rozpis ' || case z.typ when 'r' then 'ranní' when 'o' then 'odpolední' else 'celá' end
