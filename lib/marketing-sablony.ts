 ...
 *
 * KDE TO LEŽÍ VE FOODTABU
 *
 * Tenhle soubor je KATALOG DOPORUČENÝCH šablon — produktová data, ne
 * data zákazníka. Firma si je jedním tlačítkem načte do
 * `marketing_sablony`, a od té chvíle jsou to její řádky: může je
 * upravit, vypnout i smazat.
 *
 * Proto to neodporuje pravidlu 1 („nic o provozu nepatří do kódu"):
 * co zákazník mění, je řádek v databázi. V kódu je jen nabídka, ze
 * které si vybírá — jako katalog poskytovatelů, ne jako jeho menu.
 */
import type { FormatKey } from './marketing-formaty.ts'
...
/**
 * Šablona jako řádek `marketing_sablony`.
 *
 * Sloupce jsou v databázi od `20260909180000_marketing_podklady.sql` —
 * `rozvrzeni`, `pravidla_textu`, `osnova` a `formaty` čekaly prázdné
 * přesně na tohle.
 *
 * `aktivni` se schválně nenastavuje: bere se výchozí `true`. Kdyby se
 * tu psalo natvrdo, přepsalo by to při opakovaném načtení šablonu,
 * kterou si firma vypnula.
 */
export function jakoRadek(def: SablonaDef, tenantId: string, poradi: number): Record<string, unknown> {
  return {
    tenant_id: tenantId,
    klic: def.key,
    nazev: def.name,
    popis: def.description,
    ucel: def.purpose,
    pilir: def.pillar,
    rozvrzeni: def.layout,
    pravidla_textu: {
      ...def.textRules,
      vstupy: def.inputs,
...
/**
 * Katalog v pořadí, v jakém se má nabízet.
 *
 * Menu první: je to to, co restaurace zveřejňuje nejčastěji. Akce
 * uprostřed, průběžný obsah nakonec.
 */
export function doporuceneRadky(tenantId: string): Record<string, unknown>[] {
  const poradiKategorie: Record<Kategorie, number> = { menu: 0, akce: 1000, prubezne: 2000 }
  return SABLONY.map((def, i) => jakoRadek(def, tenantId, poradiKategorie[def.category] + i))
}
